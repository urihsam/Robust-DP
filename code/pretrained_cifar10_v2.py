...
with g.as_default():
    ...
    pre_trained = nets.VGG19(x_, is_training=is_training, classes=1000, use_logits=True, vanillia=True)
    ...
    with tf.variable_scope('top_layers') as opt_scope_1:
        w_1 = tf.get_variable(initializer=initializer, shape=(pre_trained_cnn.get_shape().as_list()[-1], 100), name="w_1")
        b_1 = tf.get_variable(initializer=tf.zeros_initializer(), shape=(100), name="b_1")
        w_3 = tf.get_variable(initializer=initializer, shape=(100, 10), name="w_3")
        ...
        # decay
        lr = tf.train.exponential_decay(lr, global_step, 20000, 0.99)
