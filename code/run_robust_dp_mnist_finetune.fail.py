...
def test_info(sess, model, is_finetune, test_writer, graph_dict, dp_info, log_file, total_batch=None, valid=False):
    ...
    for idx in range(total_batch):
        ...
        if is_finetune:
            ...
            batch_pretrain = sess.run(fetches=model.pre_conv, feed_dict=feed_dict)
            ...
            feed_dict = {
                ...
                graph_dict["noised_pre_holder"]: batch_pretrain+noise,
                graph_dict["label_holder"]: batch_ys,
                graph_dict["loss_coef_holder"]: FLAGS.FINETUNE_BETA,
                graph_dict["sgd_sigma_holder"]: 0.0,
                graph_dict["trans_sigma_holder"]: 0.0,
                graph_dict["is_training"]: False
            }
            ...
            feed_dict = {
                graph_dict["data_holder"]: batch_xs,
                graph_dict["label_holder"]: batch_ys,
                graph_dict["loss_coef_holder"]: FLAGS.BETA,
                graph_dict["sgd_sigma_holder"]: 0.0,
                graph_dict["trans_sigma_holder"]: 0.0,
                graph_dict["is_training"]: False
            }
...
def finetune(model=None, priv_accountant=None, model_ckpt_name=None):
    """
    """
    import pdb; pdb.set_trace()
    import time
    ...
    with g.as_default():
        ...
        if priv_accountant == None:
            print("Init a privacy accountant")
        ...
        for epoch in range(FLAGS.NUM_FINETUNE_EPOCHS):
            ...
            #for train_idx in range(1):
                ...
                for batch_idx in range(FLAGS.BATCHES_PER_LOT):
                    ...
                    batch_pre = sess.run(fetches=model.pre_conv, feed_dict=feed_dict)
                    #batch_xs = np.tile(batch_xs, [1,1,1,3])
                    noise = np.random.normal(loc=0.0, scale=input_sigma, size=batch_pre.shape)
                    feed_dict = {
                        noise_holder: noise,
                ...
                # optimization
                fetches = [model_finetune_loss, model_finetune_acc, model_finetune_lr]
                loss, acc, lr = sess.run(fetches=fetches, feed_dict=feed_dict)
                #import pdb; pdb.set_trace()
                spent_eps_delta, selected_moment_orders = priv_accountant.get_privacy_spent(sess, target_eps=[total_finetune_dp_epsilon])
                ...
                # Print info
                if train_idx % FLAGS.EVAL_FINETUNE_TRAIN_FREQUENCY == (FLAGS.EVAL_FINETUNE_TRAIN_FREQUENCY - 1):
                    ...
                    print("Loss: {:.4f}, Accuracy: {:.4f}".format(loss, acc))
                    print("Total dp eps: {:.4f}, total dp delta: {:.8f}, total dp sigma: {:.4f}, input sigma: {:.4f}".format(
                        spent_eps_delta.spent_eps, spent_eps_delta.spent_delta, total_finetune_dp_sigma, input_sigma))
                    print()
            ...
            if epoch % FLAGS.EVAL_FINETUNE_VALID_FREQUENCY == (FLAGS.EVAL_FINETUNE_VALID_FREQUENCY - 1):
            #if epoch >= 0:
                # validation
                print("\n******************************************************************")
                print("Validation")
            ...
            if epoch % FLAGS.TOTAL_FINETUNE_DP_SIGMA_DECAY_EPOCH == FLAGS.TOTAL_FINETUNE_DP_SIGMA_DECAY_EPOCH - 1:
                total_finetune_dp_sigma = max(model_utils.change_coef(total_finetune_dp_sigma, FLAGS.TOTAL_FINETUNE_DP_SIGMA_DECAY_RATE), FLAGS.MIN_TOTAL_FINETUNE_DP_SIGMA)
...
def train():
    ...
    return model, priv_accountant, ckpt_name
