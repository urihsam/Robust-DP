import nn.robust_dp_denoiser_mnist_v1 as model_mnist
import os, math
...
def test_info(sess, model, is_valid, graph_dict, dp_info, log_file, total_batch=None):
    ...
    for b_idx in range(total_batch):
        ...
        feed_dict = {
            ...
            graph_dict["noised_data_holder"]: np.clip(batch_xs+noise, 0.0, 1.0),
...
def robust_info(sess, model, recon_outputs, graph_dict, log_file):
    ...
    if not FLAGS.LOAD_ADVS:
        ...
        advs["ys"] = ys
        np.save("adv_examples_mnist_atk{}.npy".format(FLAGS.ATTACK_NORM_BOUND), advs)
        advs.pop("ys")
    else:
        advs = np.load("adv_examples_mnist_atk{}.npy".format(FLAGS.ATTACK_NORM_BOUND), allow_pickle=True).item()
        ys = advs["ys"]
        advs.pop("ys")
...
def test():
    ...
    with g.as_default():
        ...
        data_infer_holder = tf.placeholder(tf.float32, [batch_size, 28, 28, FLAGS.NUM_CHANNELS])
        recon_infer_holder = tf.placeholder(tf.float32, [batch_size, 28, 28, FLAGS.NUM_CHANNELS])
        ...
        # model
        model = model_mnist.DP_DENOISER(data=data_holder, label=label_holder, is_training=is_training, noised_data=noised_data_holder,
    ...
    with tf.Session(config=config, graph=g) as sess:
        ...
        if os.path.exists(FLAGS.DP_INFO_NPY):
            dp_info = np.load(FLAGS.DP_INFO_NPY, allow_pickle=True).item()
        else:
            dp_info = {
                "eps": FLAGS.TOTAL_DP_EPSILON,
                "delta":FLAGS.TOTAL_DP_DELTA, 
                "total_sigma":FLAGS.TOTAL_DP_SIGMA,
                "input_sigma": FLAGS.INFER_INPUT_SIGMA
            }
...
def train():
    ...
    with g.as_default():
        ...
        # training
        # OPT  
        model_op_1, model_lot_op_1, _, model_lr_1 = model.optimization(model_loss+model_reg_enc+model_reg_dec, 
                        opt_vars=enc_opt_vars+dec_opt_vars, accum_iters=FLAGS.BATCHES_PER_LOT, scope="OPT_1")
        model_zero_op_1, model_accum_op_1, model_avg_op_1 = model_lot_op_1
             
        model_op_0, model_lot_op_0, _, model_lr_0 = model.optimization(model_loss+model_reg_enc_bott,
                        opt_vars=enc_bott_opt_vars, accum_iters=FLAGS.BATCHES_PER_LOT, scope="OPT_0")
        model_zero_op_0, model_accum_op_0, model_avg_op_0 = model_lot_op_0

        model_op_2, model_lot_op_2, _, model_lr_2 = model.optimization(model_loss+model_reg_dec_top, 
                        opt_vars=dec_top_opt_vars, accum_iters=FLAGS.BATCHES_PER_LOT, scope="OPT_2")
            ...
            #for train_idx in range(1):
                ...
                for _ in range(FLAGS.BATCHES_PER_LOT):
                    ...
                    feed_dict = {
                        ...
                        noised_data_holder: np.clip(batch_xs+noise, 0.0, 1.0),
