import nn.robust_dp_mnist_transfer as model_mnist
import os, math
...
def test_info(sess, model, test_writer, graph_dict, dp_info, log_file, total_batch=None, valid=False):
    ...
    for idx in range(total_batch):
        ...
        batch_pretrain = sess.run(fetches=model.pretrain_cnn, feed_dict=feed_dict)
...
def train():
    ...
    with g.as_default():
        ...
        noised_pretrain_holder = tf.placeholder(tf.float32, [FLAGS.BATCH_SIZE, 128])
        noise_holder = tf.placeholder(tf.float32, [FLAGS.BATCH_SIZE, 128])
        ...
        is_training = tf.placeholder(tf.bool, ())
        # model
        model = model_mnist.RDPCNN(data=data_holder, label=label_holder, input_sigma=input_sigma, is_training=is_training, noised_pretrain=noised_pretrain_holder, noise=noise_holder)
        ...
        # analysis
        model_M, model_sens = model.compute_M_from_input_perturbation(model_loss_clean, FLAGS.DP_GRAD_CLIPPING_L2NORM, is_layerwised=FLAGS.IS_MGM_LAYERWISED)
        model_acc = model.cnn_accuracy
