...
def _gaussian_mech_sigma(eps, delta, sensitivity, robust_size):
    s = np.log(np.sqrt(2.0/np.pi) * 1.0/delta)
    sigma = np.sqrt(2.0 * np.log(1.25 / delta)) * sensitivity * robust_size / eps
    return sigma

def _gaussian_mech_robust_size(sigma, eps, delta, sensitivity):
    s = np.log(np.sqrt(2.0/np.pi) * 1.0/delta)
    robust_size = sigma * eps / (np.sqrt(2.0 * np.log(1.25 / delta)) * sensitivity)
    return robust_size
...
def _gaussian_robustness_size(p_max_lb, p_sec_ub, attack_size, dp_epsilon, dp_delta):
    ...
    '''
    max_r is a ratio: origin (dp_delta, dp epsilon) / minimum (dp_delta, dp epsilon)
    '''
    return max_r

def _gaussian_robustness_size_v2(p_max_lb, p_sec_ub, sigma, sensitivity, attack_size, max_eps=1.0):
    if p_max_lb <= p_sec_ub:
        # we're not even robust to the measurement error...
    ...
    #  epsilon_range = list(np.arange(0.1, 1.00000001, 0.001))  # we want 1 included
    for delta in delta_range:
        eps_min, eps_max, eps = (0.0, max_eps, max_eps/2.0)
        while eps_min < eps and eps_max >= eps:
        #  for eps in epsilon_range:
            r = _gaussian_mech_robust_size(sigma, eps, delta, sensitivity)
            if p_max_lb >= math.e ** (2 * eps) * p_sec_ub + (1 + math.e ** eps) * delta:
                if r > max_r:
                    max_r = r
                    max_r_eps = eps
...
def gaussian_robustness_size(counts, eta,
                           dp_attack_size, sigma, sensitivity=1, dp_mechanism="gaussian"):
    """Robustness size for E(argmax(f(x))) predictions (Bernoulli variable).
    ...
    count_tot = sum(counts)
    count2, count1 = sorted(counts)[-2:]
    p_max_lb = clopper_pearson_lower_bound(count1, count_tot, eta, bonferroni_hyp_n=hyp_n)
    p_sec_ub = clopper_pearson_upper_bound(count2, count_tot, eta, bonferroni_hyp_n=hyp_n)

    if dp_mechanism == 'gaussian':
        return _gaussian_robustness_size_v2(p_max_lb, p_sec_ub, sigma, sensitivity, dp_attack_size, max_eps=1.0)
    else:
        raise ValueError('Only supports the following DP mechanisms: laplace gaussian.')
