...
class MomentsAccountant(object):
  ...
  def _compute_delta(self, log_moments, eps):
    ...
    for moment_order, log_moment in log_moments:
      ...
      if log_moment < moment_order * eps:
        ...
        if min_delta > math.exp(log_moment - moment_order * eps):
          min_delta = math.exp(log_moment - moment_order * eps)
          selected_moment_order = moment_order
    return min_delta, selected_moment_order
  ...
  def _compute_eps(self, log_moments, delta):
    ...
    for moment_order, log_moment in log_moments:
      ...
      if min_eps > (log_moment - math.log(delta)) / moment_order:
        min_eps = (log_moment - math.log(delta)) / moment_order
        selected_moment_order = moment_order
    return min_eps, selected_moment_order
  ...
  def get_privacy_spent(self, sess, target_eps=None, target_deltas=None):
    ...
    if target_eps is not None:
      for eps in target_eps:
        delta, selected_moment_order = self._compute_delta(log_moments_with_order, eps)
        eps_deltas.append(EpsDelta(eps, delta))
        selected_moment_orders.append(selected_moment_order)
    else:
      assert target_deltas
      for delta in target_deltas:
        eps, selected_moment_order = self._compute_eps(log_moments_with_order, delta)
        eps_deltas.append(EpsDelta(eps, delta))
        selected_moment_orders.append(selected_moment_order)
    return eps_deltas, selected_moment_orders
