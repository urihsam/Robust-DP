...
def test_info(sess, model, is_valid, graph_dict, dp_info, log_file, total_batch=None):
    ...
    top_2_opt_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, model.opt_scope_2.name)
    bott_opt_vars = []
    ...
    for b_idx in range(total_batch):
        ...
        fetches = [model_loss, model_loss_bott, model_loss_reg_bott, model_loss_reg_1, model_loss_reg_2, model_acc]
        feed_dict[graph_dict["sgd_sigma_holder"]] = 0.0
        feed_dict[graph_dict["trans_sigma_holder"]] = 0.0

        batch_loss, batch_loss_bott, batch_reg0, batch_reg1, batch_reg2, batch_acc = sess.run(fetches=fetches, feed_dict=feed_dict)
        acc += batch_acc
        reg0 += batch_reg0
        reg1 += batch_reg1
        reg2 += batch_reg2
    ...
    # Print info
    print("Loss: {:.4f}, Loss Bott: {:.4f}, Reg loss bott: {:.4f}, Reg loss 1: {:.4f}, Reg loss 2: {:.4f}, Accuracy: {:.4f}".format(loss, loss_bott, reg0, reg1, reg2, acc))
    ...
    with open(log_file, "a+") as file: 
        ...
        file.write("Loss: {:.4f}, Loss Bott: {:.4f}, Reg loss bott: {:.4f}, Reg loss 1: {:.4f}, Reg loss 2: {:.4f}, Accuracy: {:.4f}\n".format(loss, loss_bott, reg0, reg1, reg2, acc))
...
def train():
    ...
    with g.as_default():
        ...
        gaussian_sanitizer_2 = sanitizer.AmortizedGaussianSanitizer(priv_accountant,
            [FLAGS.DP_GRAD_CLIPPING_L2NORM_2, True])
        ...
        model_op_1, _ = model.dp_optimization([model_loss, model_loss_reg_1], gaussian_sanitizer_1, sgd_sigma_holder,
                        ...
                        batched_per_lot=FLAGS.BATCHES_PER_LOT, is_layerwised=FLAGS.IS_MGM_LAYERWISED)
        model_op_2, model_lr = model.dp_optimization([model_loss, model_loss_reg_2], gaussian_sanitizer_2, sgd_sigma_holder,
                        trans_sigma=trans_sigma_holder, opt_vars=top_2_opt_vars, 
        ...
        model_M_2, _ = model.compute_M_from_input_perturbation([model_loss_clean, model_loss_reg_2], FLAGS.DP_GRAD_CLIPPING_L2NORM_2, 
                        var_list=top_2_opt_vars, is_layerwised=FLAGS.IS_MGM_LAYERWISED)
        model_acc = model.cnn_accuracy
    ...
    with tf.Session(config=config, graph=g) as sess:
        ...
        for epoch in range(FLAGS.NUM_EPOCHS):
            ...
            #for train_idx in range(1):
                ...
                min_S_min_2, sgd_sigma_2, sigma_trans_2 = cal_sigmas(lot_M, input_sigma, FLAGS.DP_GRAD_CLIPPING_L2NORM_2)
                if train_idx % 1 == 0:
                    print("top_2_layers:")
                ...
                # optimization
                fetches = [model_loss, model_loss_bott, model_loss_reg_bott, model_loss_reg_1, model_loss_reg_2, model_acc, model_lr]
                loss, loss_bott, reg0, reg1, reg2, acc, lr = sess.run(fetches=fetches, feed_dict=feed_dict)
                #import pdb; pdb.set_trace()
