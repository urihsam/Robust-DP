import nn.robust_dp_mnist_pretrained as model_mnist
import os, math
...
def __compute_S_min_2(J, param_size):
    K = np.linalg.pinv(J)
    #import pdb; pdb.set_trace()
    u, s, vt = np.linalg.svd(J)
    #import pdb; pdb.set_trace()
    zero = np.zeros((param_size))
    zero[:len(s)] = 10
    s_ = np.diag(zero)
    M = np.matmul(np.matmul(u, s_), np.transpose(u))
    return __compute_S_min(M, param_size)

def __compute_S_min_from_Jac(Jac):
    #import pdb; pdb.set_trace()
    ...
    for b in range(batch_size):
        ...
        M = np.matmul(J, np.transpose(J))
        #import pdb; pdb.set_trace()
        S_min = __compute_S_min(M, param_size)#, sen_)
        #S_min = __compute_S_min_2(J, param_size)
        batch_S_min.append(S_min)
    batch_S_min = np.stack(batch_S_min, axis=0)
    '''
    M = np.matmul(Jac, np.transpose(Jac, (0, 2, 1)))
    M = np.mean(M, axis=0) / batch_size
    #import pdb; pdb.set_trace()
    S_min = __compute_S_min(M, param_size)
    #batch_S_min = __compute_S_min_lower_bound_1(M, param_size)

    return S_min

def compute_S_min_from_Jac(Jac, is_layerwised=True):
    if is_layerwised:
        batch_S_min_layerwised = []
        for J in Jac:
            batch_S_min_layerwised.append(__compute_S_min_from_Jac(J))
        batch_S_min_layerwised = np.stack(batch_S_min_layerwised, axis=1)
        return batch_S_min_layerwised
    else:
        return __compute_S_min_from_Jac(Jac)
...
def train():
    ...
    with tf.Session(config=config, graph=g) as sess:
        ...
        if FLAGS.load_model:
            print("CNN loaded.")
            model.tf_load(sess, scope="CNN", name=FLAGS.CNN_CKPT_RESTORE_NAME)
        ...
        for epoch in range(FLAGS.NUM_EPOCHS):
            ...
            #'''
            for train_idx in range(total_train_lot//100):
                #import pdb; pdb.set_trace()
                for batch_idx in range(FLAGS.BATCHES_PER_LOT):
                    ...
                    batch_M = sess.run(fetches=model_Jac, feed_dict=feed_dict)
                    ...
                    if FLAGS.IS_MGM_LAYERWISED: # batch_K_norm is [b, #layer]
                        ...
                        min_S_min = min(min_S_min, batch_S_min)
