...
class RDPCNN:
    ...
    def __init__(self, data, label, input_sigma, is_training, noised_pretrain=None, noise=None):
        self.label = label
        self.data = tf.image.resize(data, [224, 224], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
        #self.data = data
        ...
        with tf.variable_scope('top_layers_new') as opt_scope_1:
            ...
            w_2 = tf.get_variable(initializer=initializer, shape=(32, 100), name="w_2")
            b_2 = tf.get_variable(initializer=tf.zeros_initializer(), shape=(100), name="b_2")
    ...
    @lazy_method
    def loss_reg(self):
        opt_vars_1 = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, self.opt_scope_1.name)
        #opt_vars_2 = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, self.opt_scope_2.name)
        opt_vars = opt_vars_1 #+ opt_vars_2
    ...
    def compute_M_from_input_perturbation(self, loss_list, l2norm_bound, is_layerwised=False, scope="DP_S_MIN"):
        with tf.variable_scope(scope):
            ex = self.noised_pre
            var_list_1 = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, self.opt_scope_1.name)
            #var_list_2 = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, self.opt_scope_2.name)
    ...
    def vars_size(self):
        return len(tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, self.opt_scope_1.name)) #+tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, self.opt_scope_2.name))
    ...
    def tf_load_pretrained(self, sess, name):
        print("pre trained layers loaded.")
        ex_var_list = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, self.opt_scope_1.name)
        var_list = []
