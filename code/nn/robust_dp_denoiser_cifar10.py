...
class DP_DENOISER:
    ...
    def __init__(self, data, label, is_training, noised_data=None):
        self.data = data
        self.label = label
        ...
        with tf.variable_scope("DP_DENOISER", reuse=reuse_param):
            with tf.variable_scope('AUTOENCODER', reuse=reuse_param) as ae_scope:
                ...
                with tf.variable_scope("BOTT_ENC", reuse=reuse_param) as enc_bott_scope:
                    w_0 = tf.get_variable(initializer=initializer, shape=(4, 4, 3, 32), name="W_conv_0",)
                    ...
                    net = ne.drop_out(net, 0.5, self.is_training)
                    conv_0 = ne.batch_norm(net, self.is_training)
                ...
                with tf.variable_scope("ENC", reuse=reuse_param) as enc_scope:
                    ...
                    w_2 = tf.get_variable(initializer=initializer, shape=(4, 4, 32, 64), name="W_conv_2")
                    ...
                    w_4 = tf.get_variable(initializer=initializer, shape=(4, 4, 64, 128), name="W_conv_4")
                    ...
                    net = ne.drop_out(net, 0.5, self.is_training)
                    net += res
                    conv_1 = ne.batch_norm(net, self.is_training)

                    net = tf.nn.conv2d(conv_1, w_2, strides=[1, 2, 2, 1], padding="SAME")+ b_2
                    net = tf.nn.leaky_relu(net)
                    net = ne.drop_out(net, 0.5, self.is_training)
                    conv_2 = ne.batch_norm(net, self.is_training)
                    ...
                    net = ne.drop_out(net, 0.5, self.is_training)
                    net += res
                    conv_3 = ne.batch_norm(net, self.is_training)


                    net = tf.nn.conv2d(conv_3, w_4, strides=[1, 2, 2, 1], padding="SAME")+ b_4
                    net = tf.nn.leaky_relu(net)
                    net = ne.drop_out(net, 0.5, self.is_training)
                    conv_4 = ne.batch_norm(net, self.is_training)
                    ...
                    net = ne.drop_out(net, 0.5, self.is_training)
                    net += res
                    conv_5 = ne.batch_norm(net, self.is_training)

                    '''
                    net = tf.nn.conv2d(conv_5, w_6, strides=[1, 2, 2, 1], padding="SAME")+ b_6
                    net = tf.nn.leaky_relu(net)
                    net = ne.drop_out(net, 0.5, self.is_training)
                    conv_6 = ne.batch_norm(net, self.is_training)
                    ...
                    net = ne.drop_out(net, 0.5, self.is_training)
                    net += res
                    conv_7 = ne.batch_norm(net, self.is_training)
                    '''

                    #net = tf.nn.conv2d(conv_7, w_8, strides=[1, 2, 2, 1], padding="SAME")+ b_8
                    net = tf.nn.conv2d(conv_5, w_8, strides=[1, 2, 2, 1], padding="SAME")+ b_8
                    net = tf.nn.leaky_relu(net)
                    net = ne.drop_out(net, 0.5, self.is_training)
                    conv_8 = ne.batch_norm(net, self.is_training)
                ...
                with tf.variable_scope("DEC", reuse=reuse_param) as dec_scope:
                    ...
                    w_4 = tf.get_variable(initializer=initializer, shape=(4, 4, 64, 128), name="W_decv_4")
                    b_4 = tf.get_variable(initializer=tf.zeros_initializer(), shape=(64), name="b_decv_4")
                    w_5 = tf.get_variable(initializer=initializer, shape=(3, 3, 64, 64), name="W_decv_5")
                    b_5 = tf.get_variable(initializer=tf.zeros_initializer(), shape=(64), name="b_decv_5")
                    w_6 = tf.get_variable(initializer=initializer, shape=(4, 4, 32, 64), name="W_decv_6")
                    ...
                    net = tf.nn.leaky_relu(net)
                    net = ne.drop_out(net, 0.5, self.is_training)
                    decv_0 = ne.batch_norm(net, self.is_training)
                    ...
                    net = ne.drop_out(net, 0.5, self.is_training)
                    net += res
                    decv_1 = ne.batch_norm(net, self.is_training)
                    ...
                    net = ne.drop_out(net, 0.5, self.is_training)
                    net += conv_5
                    decv_2 = ne.batch_norm(net, self.is_training)
                    ...
                    net = ne.drop_out(net, 0.5, self.is_training)
                    net += res
                    decv_3 = ne.batch_norm(net, self.is_training)
                    '''

                    #net = ne.conv2d_transpose(decv_3, w_4, b_4, strides=[2, 2], padding="SAME")
                    net = ne.conv2d_transpose(decv_1, w_4, b_4, strides=[2, 2], padding="SAME")
                    net = tf.nn.leaky_relu(net)
                    net = ne.drop_out(net, 0.5, self.is_training)
                    net += conv_3
                    decv_4 = ne.batch_norm(net, self.is_training)
                    ...
                    net = ne.drop_out(net, 0.5, self.is_training)
                    net += res
                    decv_5 = ne.batch_norm(net, self.is_training)

                    net = ne.conv2d_transpose(decv_5, w_6, b_6, strides=[2, 2], padding="SAME")
                    net = tf.nn.leaky_relu(net)
                    net = ne.drop_out(net, 0.5, self.is_training)
                    net += conv_1
                    decv_6 = ne.batch_norm(net, self.is_training)
                    ...
                    net = ne.drop_out(net, 0.5, self.is_training)
                    net += res
                    decv_7 = ne.batch_norm(net, self.is_training)
                ...
                with tf.variable_scope("TOP_DEC", reuse=reuse_param) as dec_top_scope:
                    w_8 = tf.get_variable(initializer=initializer, shape=(4, 4, 3, 32), name="W_dec_8")
                    b_8 = tf.get_variable(initializer=tf.zeros_initializer(), shape=(3), name="b_dec_8")

                    net = ne.conv2d_transpose(decv_7, w_8, b_8, strides=[2, 2], padding="SAME")
    ...
    def __call__(self, noised_data):
        self.noised_data = noised_data
        self.recon_data = self.reconstruct(self.noised_data)
        self.recon_clean_data = self.reconstruct(self.data, True)
        

        with tf.variable_scope("cifar10cnn") as cifar10cnn_scope:
            x_ = tf.image.resize(self.recon_data*255.0, [224, 224], method=tf.image.ResizeMethod.BILINEAR)
            ...
            with tf.variable_scope('top_layers'):
                ...
                self.recon_accuracy = tf.reduce_mean(tf.cast(correct_pred, tf.float32), name='accuracy')
        self.cifar10cnn_scope = cifar10cnn_scope


        with tf.variable_scope("cifar10cnn", reuse=True) as cifar10cnn_scope:
            x_ = tf.image.resize(self.data*255.0, [224, 224], method=tf.image.ResizeMethod.BILINEAR)
            pre_trained = nets.VGG19(x_, is_training=False, classes=100, use_logits=True)
            pre_trained_cnn = tf.reshape(pre_trained, [pre_trained.get_shape().as_list()[0], -1])
            

            initializer = tf.contrib.layers.variance_scaling_initializer(factor=1.0, mode='FAN_AVG', uniform=True)
            with tf.variable_scope('top_layers', reuse=True):
                w_1 = tf.get_variable(initializer=initializer, shape=(pre_trained_cnn.get_shape().as_list()[-1], 32), name="w_1")
                b_1 = tf.get_variable(initializer=tf.zeros_initializer(), shape=(32), name="b_1")
                w_2 = tf.get_variable(initializer=initializer, shape=(32, 10), name="w_2")
                b_2 = tf.get_variable(initializer=tf.zeros_initializer(), shape=(10), name="b_2")
    ...
    def loss(self, coef):
        ...
        #out_2 = self.vectorize(ne.sigmoid(self.sliced_generated[index]))
        #loss += coef * -1 * tf.reduce_mean(tf.reduce_sum(in_ * tf.log(out_2 + 1e-3) + (1-in_) * tf.log(1-out_2 + 1e-3), 1))
    ...
    @lazy_method
    def loss_reg(self, coef, scope=None):
        if scope==None:
            scope = FLAGS.SLICE_NAME
    ...
    def compute_M_from_input_perturbation_v0(self, data, loss, l2norm_bound, var_list=None, scope="DP_S_MIN"):
        with tf.variable_scope(scope):
            ...
            for idx in range(len(loss)):
                px_grads_.append(tf.gradients(loss[idx], xs, name="grads_{}".format(idx)))
            px_grads = [tf.stack(px_) for px_ in list(zip(*px_grads_))]
            '''
    ...
    @lazy_method_no_scope
    def dp_optimization_v0(self, loss, priv_accountant, dp_sigma, act_sigma=None, opt_vars=None, batches_per_lot=1, 
