...
class RDPCNN:
    ...
    def __init__(self, data, label, input_sigma, is_training, noised_pretrain=None, noise=None):
        ...
        self.pre_trained_cnn = tf.reshape(self.pre_trained.outputs()[-3], [self.pre_trained.get_shape().as_list()[0], 32, 32, 4])
        ...
        with tf.variable_scope('top_layers') as opt_scope_1:
            ...
            #w_2 = tf.get_variable(initializer=initializer, shape=(3, 3, 4, 4), name="W_conv_2")
            #b_2 = tf.get_variable(initializer=tf.zeros_initializer(), shape=(4), name="b_conv_2")
            w_3 = tf.get_variable(initializer=initializer, shape=(4, 4, 4, 4), name="W_conv3")
            b_3 = tf.get_variable(initializer=tf.zeros_initializer(), shape=(4), name="b_conv_3")
            #w_4 = tf.get_variable(initializer=initializer, shape=(3, 3, 4, 4), name="W_conv_4")
            #b_4 = tf.get_variable(initializer=tf.zeros_initializer(), shape=(4), name="b_conv_4")
            ...
            #net = tf.nn.leaky_relu(tf.nn.conv2d(net, w_2, strides=[1, 1, 1, 1], padding="SAME")+ b_2)
            net = tf.nn.leaky_relu(tf.nn.conv2d(net, w_3, strides=[1, 2, 2, 1], padding="SAME")+ b_3)
            #net = tf.nn.leaky_relu(tf.nn.conv2d(net, w_4, strides=[1, 1, 1, 1], padding="SAME")+ b_4)
    ...
    @lazy_method
    def loss_bott(self):
        loss = FLAGS.BETA * tf.losses.softmax_cross_entropy(self.label, self.cnn_bott_logits)
        tf.summary.scalar("Total_loss_bott", loss)
        return loss


    @lazy_method
    def loss_reg(self, opt_vars=None):
        if opt_vars == None:
    ...
    @lazy_method_no_scope
    def dp_optimization(self, loss, sanitizer, dp_sigma, trans_sigma=None, opt_vars=None, batched_per_lot=1, is_sigma_data_dependent=False, is_layerwised=False, scope="DP_OPT"):
        '''
        def no_dp():
