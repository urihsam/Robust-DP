...
class DP_DENOISER:
        ...
        with tf.variable_scope("DP_DENOISER", reuse=reuse_param):
            with tf.variable_scope('AUTOENCODER', reuse=reuse_param) as ae_scope:
                ...
                with tf.variable_scope("ENC", reuse=reuse_param) as enc_scope:
                    ...
                    w_2 = tf.get_variable(initializer=initializer, shape=(4, 4, 16, 128), name="W_conv_2")
                ...
                with tf.variable_scope("DEC", reuse=reuse_param) as dec_scope:
                    ...
                    w_6 = tf.get_variable(initializer=initializer, shape=(4, 4, 16, 128), name="W_decv_6")
                    b_6 = tf.get_variable(initializer=tf.zeros_initializer(), shape=(16), name="b_decv_6")
                    w_7 = tf.get_variable(initializer=initializer, shape=(3, 3, 16, 16), name="W_decv_7")
                    ...
                    net = ne.conv2d_transpose(decv_0, w_2, b_2, strides=[2, 2], padding="SAME")
                    net = tf.nn.leaky_relu(net)
                    net = ne.drop_out(net, drop_rate, self.is_training)
                    ...
                    net = ne.conv2d_transpose(decv_2, w_4, b_4, strides=[2, 2], padding="SAME")
                    net = tf.nn.leaky_relu(net)
                    net = ne.drop_out(net, drop_rate, self.is_training)
    ...
    def prediction(self, data, reuse_param=False):
        ...
        data_infer = data*255.0
        
        with tf.variable_scope("cifar10cnn", reuse=reuse_param) as cifar10cnn_scope:
            x_ = data_infer
            pre_trained = nets.VGG16(x_, is_training=False, stem=True)
            #import pdb; pdb.set_trace()
            ...
            with tf.variable_scope('top_layers') as opt_scope_1:
                ...
                net = ne.drop_out(net, drop_rate, False)
                net = ne.layer_norm(net, False)
