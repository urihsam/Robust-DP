...
class DP_DENOISER:
    ...
    def tf_load(self, sess, scope=None, model_path="./models/MNIST/", name='sliced_ae.ckpt'):
        if scope == None:
            scope = "DP_DENOISER"
        #saver = tf.train.Saver(dict(self.conv_filters, **self.conv_biases, **self.decv_filters, **self.decv_biases))
        saver = tf.train.Saver(var_list=tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=scope))
    ...
    def tf_save(self, sess, scope=None, model_path="./models/MNIST/", name='sliced_ae.ckpt'):
        if scope == None:
            scope = "DP_DENOISER"
        #saver = tf.train.Saver(dict(self.conv_filters, **self.conv_biases, **self.decv_filters, **self.decv_biases))
        saver = tf.train.Saver(var_list=tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=scope))
